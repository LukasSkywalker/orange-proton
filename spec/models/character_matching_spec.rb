...
describe CharacterMatcher do
  before(:each) do
    @cm = CharacterMatcher.new
  end

  it "should find number of matching characters" do
    assert_equal(@cm.character_matching('helloi', 'helloi'), 2)
    assert_equal(@cm.character_matching('helloi', 'hello hello'), 2)
  ...
  it "should find length of matching sequence" do
    assert_equal(@cm.sequence_matching('helloi', 'helloi'), 6)
    assert_equal(@cm.sequence_matching('helloi', 'hello hello'), 5)

    assert_equal(@cm.sequence_matching('kardiologie', 'endokrinologie'), 0)
    assert_equal(@cm.sequence_matching('kardiographie', 'pathologie'), 0)
    assert_equal(@cm.sequence_matching('sportmedizin', 'tauchmedizin'), 1)
    assert_equal(@cm.sequence_matching('psychotherapie', 'sporttherapie'), 1)
    assert_equal(@cm.sequence_matching('allgemeinchirurgie', 'augenchirurgie'), 1)
    assert_equal(@cm.sequence_matching('gastroskopie', 'teleskopie'), 1)
    assert_equal(@cm.sequence_matching('psychiatrie', 'entwicklungspaediatrie'), 0)
    assert_equal(@cm.sequence_matching('homoeopathie', 'animalopathie'), 0)
